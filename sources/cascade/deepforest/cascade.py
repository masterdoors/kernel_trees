...
class BaseCascadeForest(BaseEstimator, metaclass=ABCMeta):
    ...
    def _make_layer(self, **layer_args):
        ...
        if not hasattr(self, "use_custom_estimator"):
            ...
            if hasattr(self, "use_win")and layer_args['kernel_size'] > -1:
                # Use customized cascade layers
                layer = WinCascadeLayer(
                    layer_args["layer_idx"],
                    self.n_splits,
                    layer_args["n_outputs"],
                    self.dummy_estimators,
                    layer_args['kernel_size'],
                    layer_args['stride'],
                    layer_args['sizex'],
                    layer_args['sizey'],
                    layer_args['padding'],                    
                    layer_args['filter_size'],
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if hasattr(self, "use_win"):
                ...
                layer_ = self._make_layer(
                    ...
                    global_pool=self.global_pool[0]
                )         
        else:        
            layer_ = self._make_layer(
                layer_idx=0,
        ...
        LOG(str(self._repr_performance(pivot)))    
        ...
        while self.n_layers_ < self.max_layers:
            ...
            if hasattr(self, "use_win"):
                    ...
                    layer_ = self._make_layer(
                        ...
                        sizex = self.sizex[layer_idx],
                        sizey = self.sizey[layer_idx],
                        filter_size = self.filter_size[layer_idx],       
                        padding = self.padding[layer_idx],                    
                        ...
                        random_state=self.random_state,
                        verbose=self.verbose,
                        global_pool=self.global_pool[layer_idx]                        
                    )         
            else:             
                layer_ = self._make_layer(
                    layer_idx=layer_idx,
    ...
    def set_win(self,kernel_size,stride,sizex,sizey,padding,filter_size,global_pool):
        self.use_win = True
        #Win
