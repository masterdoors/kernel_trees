...
class WinCascadeLayer(CustomCascadeLayer):
    ...
    def fit_transform(self, X, y, sample_weight=None): 
        #for i in range(28):
        #    for j in range(28):
        #        print (format(int(X[0,i*28+j]/30),'01d'),end='')
        #    print('') 
        #print(y[0])    
        #print('')
        #print("???????????") 

        X_patch, y_patch = self.scanner.fit_transform(X,y)
        #print(X.shape)
        #print(X_patch.shape)
        #print('')
        #for i in range(4):
        #    for j in range(27):
        #        for k in range(27):
        #            print (format(int(X_patch[i,j*27+k]/30),'01d'),end='')
        #        print('')            
        #    print("!!!!!")
        #    print(y_patch[i])
        #    print('')
        X_aug_patch = super().fit_transform(X_patch, y_patch, sample_weight=None)
        num_win = int(X_patch.shape[0] / X.shape[0])
        slices = []
        if self.global_pool:
            for i in range(X_aug_patch.shape[1]):
                slices.append(X_aug_patch[:,i].reshape(X.shape[0],num_win).max(axis=1).reshape(-1,1))
        else:
            for i in range(X_aug_patch.shape[1]):
                slices.append(X_aug_patch[:,i].reshape(X.shape[0],-1,self.filter_size).max(axis=2))
                
        v = np.hstack(slices)            
        return v
    
    def predict_full(self, X):
        X_patch = self.scanner.transform(X)        
        X_aug_patch = super().predict_full(X_patch)
        num_win = int(X_patch.shape[0] / X.shape[0])
        slices = []
        if self.global_pool:
            for i in range(X_aug_patch.shape[1]):
