...
#public:
    ...
    #    if sample:
    #        x = x[:,self.features_weight]
    #    numpy.save("vertexDataX",x.data)
    #    numpy.save("vertexIndX",x.indices)
    #    numpy.save("vertexPtrX",x.indptr)
        
    #    with open('shape.pickle', 'wb') as f:
    #        pickle.dump(x.shape, f)            
            
        #self.model.save_to_file("vertex.model")
    #    p = subprocess.Popen("python predict.py " + self.classifier_id, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True)
    #    p.wait()
    #    signs = numpy.load('sign.npy')        
    #    return sign(signs)
