...
@author: keen
'''
import decision_stamp_GPU as dst
            ...
            if nnz >= self.min_samples_split: 
                if not self.clearNode(Y, sample_weight):
                    ...
                    ds = dst.DecisionStamp(self.n_classes,self.class_max, features_weight,\
                                           ...
                                           self.dual,C/cf,tol,self.max_iter,self.gamma,self.intercept_scaling,self.dropout_low,self.dropout_high,balanced,self.noise,self.cov_dr, self.criteria,self.tree_number)
            ...
            if Y.shape[0] > 0 and x.shape[0] == Y.shape[0]:
                ...
                for i,s in enumerate(self.structure):
                    if s[0] == -1 or s[1] == -1:
                        ...
                        if s[0] + s[1] == -2:
                            leaf_id += 2
                            self.adj_leaves[leaf_id] = [self.nodes[i].instability,1./(self.nodes[i].probL + 0.001),1./(self.nodes[i].probR + 0.001)]                            
                        else:
                            leaf_id += 1
